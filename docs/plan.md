...
- **Resolved: per-vendor credentials live directly on the Vendor module, no
  ...
  number of dev stores, but only one real one) — confirmed by trying to
  point one app's distribution at a second vendor's domain and finding no
  way to do it. So **staff creates one Shopify app per vendor**, not one
  shared app with a link generated per vendor as originally assumed. Storage
  follows the same pattern already used for vendor-user passwords: staff
  pastes the vendor's `shopify_store_domain` / `shopify_client_id` /
  `shopify_client_secret` onto the Vendor record via the existing
  `PATCH /admin/vendors/:id`, Shopify's OAuth callback fills in
...
- **V1 connects a vendor's Shopify with a private custom-distribution app,
  created by staff per vendor — not a public Shopify app.** Originally
  ...
  *app* per vendor, not one link per vendor from a shared app. Still not a
  ...
  onboarding entry above). Creating one more Shopify app is free and
  instant either way, just a repeated few minutes of clicking
  (`docs/vendor-shopify-connection-guide.md`). This wasn't put to Sensus as
  a question — their original proposal already implied a manual install and
