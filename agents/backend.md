...
- `workflows/`: orchestration; prefer over fat route handlers. **Every route, including GETs, calls exactly one workflow** — a deliberate departure from Medusa's own default (a plain read may call `query.graph` straight from the route); this codebase's reads are not simple gets, they join across multiple `query.graph` calls, apply vendor-ownership scoping, and do real response-shaping, so they get the same route→workflow discipline as a mutation.
  ...
  **Call Query through `graph` (`src/lib/query.ts`), not `query.graph` directly.** It is a pure type-level wrapper — it resolves nothing, so the call site still resolves Query itself and passes it in:
  ...
  Its first parameter is `Pick<RemoteQueryFunction, "graph">` — only the method it uses — so future `gql`/`index` helpers live beside it in the same file, each taking its own slice. All 57 call sites use it.

  Why it exists: `medusa build`/`medusa dev` regenerates `.medusa/types/query-entry-points.d.ts`, so _result_ typing already works on raw `query.graph`, but _input_ typing does not — Medusa types `entity` as `TEntry | keyof RemoteQueryEntryPoints` and `fields` as `...[] | string[]`, and each `| string` widens the contextual type to nothing the editor can suggest. `graph` narrows `entity` to `QueryEntity` (a closed set, so a typo is a compile error with a "Did you mean" hint) and keeps the result typed, while `fields` is `SuggestedField<T> | (string & {})`: the literal union drives completions (254 entities, 451 fields on `affiliate`, 1607 on `product`, including `*` and `relation.*`) but any string still assigns.

  **Do not "tighten" that `(string & {})`.** Query resolves field paths of unbounded depth — the docs' own example is `service_zone.fulfillment_set.location.id`, and `variants.prices.*` (documented, live in `build-vendor-product-detail.ts`) is absent from the generated types entirely because it resolves through a module link. A strict version was written and rejected five patterns already in this repo. Generated types describe what the generator knew, not what the runtime accepts.

  A dynamic `entity` must be typed `QueryEntity` at its source rather than left as `string` (see `queryEntity` threaded through `update-storefront-content.ts` → `upsert-storefront-content.ts` → `storefront-content-route.ts`); a literal passed through `transform()` needs `as const`, since `transform` widens. Enforcing `entity` is worth that friction — it immediately caught `entity: ""` committed in `get-affiliate-me.ts`, which had silently broken the affiliate `/me` query.

  Two traps if you edit `src/lib/query.ts`: `entity: TEntity` must be declared directly, not left inside `RemoteQueryInput<TEntity>`, because a conditional type is a non-inferrable position and `TEntity` silently collapses to `any` (everything compiles, nothing is checked); and the single `as` cast is unavoidable because TS cannot resolve that conditional inside a generic body — it only widens, so it is safe, and is not what the no-`as` rule above targets. Cost is negligible: +439 types / +177 instantiations across 10 entities. The Zod-parse rule still applies — field _names_ are suggested, the runtime shape is still unvalidated.

  **`src/admin` is excluded from the backend `tsconfig.json` and has its own `src/admin/tsconfig.json`.** A bare `tsc --noEmit` therefore checks none of the Admin extension — forms, routes, widgets and hooks all go unverified, and a broken admin form still "typechecks". The `typecheck` script now runs both projects (`tsc --noEmit && tsc --noEmit -p src/admin/tsconfig.json`), matching what CI already did in a separate step. Never conclude the backend is type-clean from the root project alone.

  **Vendor commission is recorded on the consignment, never recomputed.** `Vendor.commission_rate` is a 0–1 fraction (same convention as `Affiliate`, enforced by the shared `commissionRateSchema` in `packages/api-contracts/src/common/commission-rate.ts`). When an order is placed, `createConsignmentsStep` looks up each vendor's rate and stores `currency_code`, `subtotal`, `commission_rate`, `commission_total` and `earning_total` on that vendor's `Consignment` — the rate is _copied_, so an old order still explains itself after the vendor's rate changes. `buildConsignmentEarnings` owns the arithmetic and always derives `earning_total` as `subtotal - commission_total` rather than rounding it separately, so the parts reconcile exactly. Nothing that moves money exists: no payouts, ledger, refunds or adjustments — see `docs/features/commission-and-payouts.md`.

  Two traps this hit. **Order line-item `quantity` lives on the order-item join**: asking `query.graph` for `items.quantity` alone silently returns nothing (while `items.unit_price` comes back fine), so `items.detail.quantity` must be requested alongside it — this shipped a silent `subtotal: 0` on every consignment until an integration test caught it. And **a new optional field on `updateVendorSchema` must be added to its `.refine()`**, or an update carrying only that field is rejected as "changes nothing".
...
- `seeds/` (app root, **not** under `src/` — it's demo/test data, not application source). Config-driven:
  - `seed-config.ts` (`SEED_CONFIG`) is the only file to edit to change what gets seeded: login name lists (`staff`/`vendors`/`affiliates` → `<name>@<role>.com`, all using `password`), entity counts, `{ min, max }` ranges per relation, product option sets, and `randomSeed`.
  - `seed-plan.ts` (`buildSeedPlan(config)`) is pure, with no Medusa imports, so `e2e/lib/seed-plan.ts` can import it. It turns the config into concrete fixtures with a faker instance seeded from `randomSeed`, so the same config always gives the same plan. Categories may share products; collections never do (a product has one `collection_id`). Orders are planned from per-vendor/per-affiliate order counts: order sizes are drawn from `vendorsPerOrder`, then filled with the vendors that have the most links left. The smallest order count is used that still exercises both ends of `vendorsPerOrder` and `affiliatesPerOrder`, so some orders have several vendors and some have no affiliate. It also asserts that no variant is ordered beyond its `stockPerVariant`. It throws `SEED_CONFIG: …` when the ranges contradict each other. Every storefront-facing entity (vendor, affiliate, category, collection) gets storefront content, with its hero from `https://picsum.photos/seed/<handle>/1200/450`, so the same handle always gets the same image. The storefront's `next/image` runs `unoptimized`, so no `remotePatterns` entry is needed.
  - `seed-<entity>.ts` each rebuild the plan and create their slice through the app's real workflows (`createVendorProductWorkflow`, `createVendorStockLocationWorkflow`, `promoteAffiliateProductWorkflow`, `createConsignmentsWorkflow`, …), never hand-inserted rows. `seed.ts` runs them in order: catalog → categories → collections → identity → vendors → affiliates → orders. Each also runs alone as `pnpm run seed:<entity>`.
  - Everything except `seed-catalog.ts` is idempotent by handle/email (orders: skipped once any consignment exists). After changing the config, `db:reset` instead of re-seeding, or old and new datasets mix.
  - `seed-identity.ts` still honours `ADMIN_EMAIL` + `ADMIN_PASSWORD` (`docs/prod-bootstrap.md`) in place of the config's staff list.
  - `seed-catalog.ts` is platform-level setup only (sales channel, publishable key, store currencies, regions, tax regions) and **not idempotent**. It reuses the migration-created store + sales channel instead of creating new ones. A second, EUR-only store made vendor product prices EUR-only, which caused a "can't add to cart" 500.
...
- **Vendor isolation cannot come from Admin.** The User Module gives users and
  ...
  **Vendor and VendorUser are two separate, independent CRUDs, not one
  ...
  and are deliberately vendor-self-service end to end: `PATCH /vendors/
  shopify/connection` and `GET /vendors/shopify/connection/install-link`
...
- **Shopify pull is spiked, not settled — `docs/spikes/vendor-shopify-sync.md`.**
  ...
  **The real product-import trigger now exists, on the vendor panel itself
  ...
  the vendor pulls their catalogue (`GET /vendors/shopify/products`, each
...
one fail for an unrelated reason. E2E fixtures (`FIRST_VENDOR`,
`FIRST_AFFILIATE`, …) are read from `buildSeedPlan(SEED_CONFIG)`, never
hardcoded, so they follow whatever the seed config produces.
