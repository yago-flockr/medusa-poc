...
Medusa v2 commerce engine for the chassis. Config: `medusa-config.ts`. Default seed markets: `src/lib/markets.ts` (UK / US regions for seed). Seed scripts themselves live in `seeds/`, a sibling of `src/` — see the `seeds/` entry below.
