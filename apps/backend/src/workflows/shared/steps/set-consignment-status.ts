...
export const setConsignmentStatusStep = createStep(
  ...
  async ({ consignmentId, status }: SetConsignmentStatusStepInput, { container }) => {
    const vendorModuleService = container.resolve(VENDOR_MODULE)
    const consignment = await vendorModuleService.retrieveConsignment(consignmentId)
    const previousStatus = consignment.status as VendorConsignmentStatus

    const updated = await vendorModuleService.updateConsignments({ id: consignmentId, status })

    return new StepResponse(updated, { consignmentId, previousStatus })
  },
  async (compensation: SetConsignmentStatusCompensation | undefined, { container }) => {
    if (!compensation) {
      return
    }
