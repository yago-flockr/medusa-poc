...
describe("buildShopifyOauthRedirectUri", () => {
  it("uses the real protocol and host for a non-localhost request", () => {
    expect(
      buildShopifyOauthRedirectUri("https", "medusa-poc.medusajs.app"),
    ).toBe("https://medusa-poc.medusajs.app/hooks/shopify/oauth/callback")
  })
...
describe("verifyShopifyCallbackHmac", () => {
  ...
  it("is independent of key order when signing", () => {
    ...
    const hmac = sign(
      { code: "abc123", shop: "test.myshopify.com", timestamp: "123" },
      secret,
    )
  ...
  it("returns false when hmac is missing", () => {
    expect(
      verifyShopifyCallbackHmac({ shop: "test.myshopify.com" }, secret),
    ).toBe(false)
  })
